...
class ExpenseItemHolder(itemView: View,val optionsClickAction:((ExpenseItem)->Unit)?) : RecyclerView.ViewHolder(itemView) {
    ...
    fun bind(expenseItem: ExpenseItem) {
        expenseItem.apply {
            ...
            tv_exp_item_unit_price_text.text = unitPrice.getCurrencyStringWithSymbol(itemView.context)
            tv_exp_item_qty_text.text = qty.get2DecPoints().toString()
            tv_exp_item_total_price_text.text = (unitPrice*qty).getCurrencyStringWithSymbol(itemView.context)
